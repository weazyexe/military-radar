/**
 * Ультра краткий гуиде как тут жить
 * $("строка") - это поиск элемента страницы в html
 * - $("#kek") - найти элемент с id="kek". id на странице всегда уникальный, не может быть 2 блока с одинаковым id
 * - $(".kek") - найти все элементы, у которых в поле class есть "kek"
 * - $(this) - используется вместе с each() функцией. each - функция, применяемая к массиву,
 *             вызывающая функцию переданную в each для каждого элемента. $(this) просто возвращает текущий элемент
 * 
 * 
 * Анимация задается через css свойство animation, там много сложных штучек что я сам не понимаю
 * 
 * 
 * Можешь поподробнее почитать про html/css в целом, если ты не помнишь чего
 */
